...
import os from 'os'
import { mergeClaude, copyRecursive, MARKER_START, MARKER_END, SKIP_DIRS_IF_EXISTS } from '../src/update'

const withTmpDir = (fn: (dir: string) => void): void => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'agent-docs-test-'))
  fn(dir)
